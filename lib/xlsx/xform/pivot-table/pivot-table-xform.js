...
class PivotTableXform extends BaseXform {
  ...
  render(xmlStream, model) {
    ...
    // rows: [0, 1],
    // columns: [2],
    // values: [{field: 4, type: 'sum'}, {field: 5, type: 'average'}],
    //
    // the field numbers are indices into `cacheFields`.
    ...
    xmlStream.writeXml(`
      ...
      <dataFields count="${values.length}">
        ${renderDataFields(values, cacheFields)}
      </dataFields>
      <pivotTableStyleInfo
...
function renderDataFields(values, cacheFields) {
  return values
    .map(value => {
      const fieldName = cacheFields[value.field].name;
      const aggregationType = value.type || 'sum';
      const displayName = getAggregationDisplayName(aggregationType, fieldName);
      ...
      return `<dataField
          ...
          baseField="0"
          baseItem="0"
          ${aggregationType !== 'sum' ? `subtotal="${aggregationType}"` : ''}
        />`;
    })
    .join('\n        ');
}

function getAggregationDisplayName(type, fieldName) {
  const displayNames = {
    average: 'Average',
    count: 'Count',
    countNums: 'Count',
  ...
  const prefix = displayNames[type] || 'Sum';
  return `${prefix} of ${fieldName}`;
}
    ...
    .map((cacheField, fieldIndex) => {
      const fieldType =
        pivotTable.rows.indexOf(fieldIndex) >= 0
          ...
          ? 'column'
          : pivotTable.values.some(v => v.field === fieldIndex)
          ? 'value'
          : null;
